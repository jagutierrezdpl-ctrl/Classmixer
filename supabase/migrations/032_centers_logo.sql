...
-- Storage policies (DROP first so the script is re-runnable)
DROP POLICY IF EXISTS "center_logo_upload" ON storage.objects;
DROP POLICY IF EXISTS "center_logo_public_read" ON storage.objects;
...
CREATE POLICY "center_logo_public_read"
